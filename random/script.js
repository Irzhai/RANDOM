...
var count = setInterval(function say() {
    i--;
    if (i == 0) {
        main.innerHTML = "zamanin bitdi";
        clearInterval(count);
    }
    time.innerHTML = "Vaxta" + i + " saniye qalib";
}, 1000);
...
document.getElementById("submit").addEventListener("click", function () {
    if (input.value != "" && input1.value != "" && number.value == netice) {
        clearInterval(count);
        document.write("Congrad");
    }
    else if (input.value == "" || input1.value == "" || number.value == "") {
        error.innerText = "Bos xana var";
    } else if (number.value != netice) {
        error.innerText = "Duzgun hesablama deyil";
    }
})
